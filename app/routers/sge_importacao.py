...
async def programas_summary(
    ...
    if regiao:
        ...
        filtros_oferta.append(
            f"UPPER(TRIM(r.nome)) = ${idx_regiao}"
        )
    ...
    if programas:
        ...
        if ids_prog:
            ...
            filtros_oferta.append(f"o.cod_programa = ANY(${idx_prog}::int[])")

            filtro_meta_programa = f"""
            AND EXISTS (
                SELECT 1
                FROM programas p
                WHERE p.codigo = ANY(${idx_prog}::int[])
                AND UPPER(TRIM(p.nome_programa)) = UPPER(TRIM(ps.programa_raw))
            )
            """
    ...
    filtro_meta_regiao = ""
    if idx_regiao:
        filtro_meta_regiao = f"""
        AND (
            UPPER(TRIM(ps.regiao)) = ${idx_regiao}
            OR UPPER(TRIM(ps.subregiao)) = ${idx_regiao}
        )
        """
    ...
    if idx_uo:
        filtro_meta_uo = f"""
        AND NULLIF(regexp_replace(COALESCE(ps.cod_uo_raw, ''), '[^0-9]', '', 'g'), '')::int = ${idx_uo}
        """
    ...
    meta AS (
        SELECT
            ...
            COALESCE(SUM(CASE WHEN UPPER(TRIM(ps.conta)) IN ('MATRÍCULAS', 'MATRICULAS') AND UPPER(TRIM(ps.financiamento_raw)) IN ('GRATUIDADE NÃO REGIMENTAL','GRATUITO')
                THEN CASE WHEN 1 = ANY(${idx_mes}::int[]) THEN COALESCE(ps.jan,0) ELSE 0 END +
                    CASE WHEN 2 = ANY(${idx_mes}::int[]) THEN COALESCE(ps.fev,0) ELSE 0 END +
                    CASE WHEN 3 = ANY(${idx_mes}::int[]) THEN COALESCE(ps.mar,0) ELSE 0 END +
                    CASE WHEN 4 = ANY(${idx_mes}::int[]) THEN COALESCE(ps.abr,0) ELSE 0 END +
            ...
            COALESCE(SUM(CASE WHEN UPPER(TRIM(ps.conta)) IN ('HORA ALUNO', 'HORA-ALUNO', 'HORA_ALUNO') AND UPPER(TRIM(ps.financiamento_raw)) IN (
    'GRATUIDADE NÃO REGIMENTAL',
    'GRATUITO'
)
                THEN
    ...
    return {
        ...
        "matriculas": {
            ...
            "meta_gr": float(d.get("meta_gr") or 0),
            "meta_g": float(d.get("meta_g") or 0),
            "meta_p": float(d.get("meta_p") or 0),
        },
        ...
        "hora_aluno": {
            ...
            "meta_gr": float(d.get("ha_meta_gr") or 0),
            "meta_g": float(d.get("ha_meta_g") or 0),
            "meta_p": float(d.get("ha_meta_p") or 0),
        },
...
@router.get("/dashboard/filtros/opcoes")
async def dashboard_filtros_opcoes(
    request: Request,
    ano: int = 2026,
    meses: str | None = None,
    programas: str | None = None,
        ...
        if ids_meses:
            ...
            filtros.append(f"""
                EXISTS (
                    ...
                    WHERE mp.cod_oferta = o.codigo
                      AND mp.ano = o.ano
                      AND mp.mes = ANY(${len(params)}::int[])
                )
            """)
    ...
    if programas:
        ...
        if ids_prog:
            params.append(ids_prog)
            filtros.append(f"o.cod_programa = ANY(${len(params)}::int[])")

    if regiao:
        params.append(regiao.strip().upper())
        filtros.append(f"UPPER(TRIM(r.nome)) = ${len(params)}")

    if subregioes:
        ids_sub = [int(x) for x in subregioes.split(",") if x.strip().isdigit()]
        if ids_sub:
            params.append(ids_sub)
            filtros.append(f"s.codigo = ANY(${len(params)}::int[])")

    if uo:
        if str(uo).strip().isdigit():
            params.append(int(uo))
            filtros.append(f"u.codigo = ${len(params)}")
    ...
    sql = f"""
        SELECT
            ...
            p.nome_programa AS programa_nome,

            r.nome AS regiao_nome,

            s.codigo AS subregiao_codigo,
            s.nome AS subregiao_nome,

            u.codigo AS uo_codigo,
            u.nome AS uo_nome
    ...
    for r in rows:
        if r["programa_codigo"] and r["programa_nome"]:
            programas_map[r["programa_codigo"]] = r["programa_nome"]

        if r["regiao_nome"]:
            regioes_set.add(r["regiao_nome"])

        if r["subregiao_codigo"] and r["subregiao_nome"]:
            subregioes_map[r["subregiao_codigo"]] = r["subregiao_nome"]

        if r["uo_codigo"] and r["uo_nome"]:
            uos_map[r["uo_codigo"]] = r["uo_nome"]

    return {
        "programas": [
            {"codigo": k, "nome": v}
            for k, v in sorted(programas_map.items(), key=lambda x: x[1])
        ],
        "regioes": [
            {"nome": v}
            for v in sorted(regioes_set)
        ],
        "subregioes": [
            {"codigo": k, "nome": v}
            for k, v in sorted(subregioes_map.items(), key=lambda x: x[1])
        ],
        "uos": [
            {"codigo": k, "nome": v}
            for k, v in sorted(uos_map.items(), key=lambda x: x[1])
        ],
    }
...
@router.get("/modalidades/financiamentos")
async def modalidades_financiamentos(
    request: Request,
    ano: int,
    meses: str | None = None,
    programa: str | None = None,
    ...
    async with pool.acquire() as conn:
        ...
        if subregioes:
            ...
            if ids_sub:
                params.append(ids_sub)
                idx = len(params)
                filtro_plan += f" AND s.codigo = ANY(${idx}::int[])"
                filtro_real += f" AND sr.codigo = ANY(${idx}::int[])"
        ...
        if programas:
            ...
            if ids_prog:
                ...
                AND EXISTS (
                    SELECT 1
                    FROM programas p
                    WHERE p.codigo = ANY(${idx}::int[])
                    AND UPPER(TRIM(p.nome_programa)) = UPPER(TRIM(ps.programa_raw))
                )
                """

                filtro_real += f" AND o.cod_programa = ANY(${idx}::int[])"

        if regiao:
            params.append(regiao.strip().upper())
            idx = len(params)

            filtro_plan += f" AND UPPER(TRIM(ps.regiao)) = ${idx}"
            filtro_real += f" AND UPPER(TRIM(r.nome)) = ${idx}"
        ...
        if uo:
            if str(uo).strip().isdigit():
                ...
                filtro_plan += f"""
                AND NULLIF(
                    regexp_replace(COALESCE(ps.cod_uo_raw, ''), '[^0-9]', '', 'g'),
                    ''
                )::int = ${idx}
                """

                filtro_real += f" AND u.codigo = ${idx}"
...
async def market_share_aprendizagem(
    ...
    aprendizes_regiao AS (
        SELECT
            sr.codigo_regiao AS cod_regiao,
            COALESCE(SUM(rp.matriculas_real), 0) AS aprendizes
        FROM realizado_programas rp
        JOIN ofertas_programas o
        ON o.codigo = rp.cod_oferta
        JOIN programas p
        ...
        JOIN uo u
        ON u.codigo = o.cod_uo
        LEFT JOIN subregioes sr
        ON sr.codigo = u.cod_subregiao
        WHERE UPPER(TRIM(p.nome_programa)) = 'JOVEM APRENDIZ'
        AND rp.ano = $1
        AND rp.mes = ANY($2::int[])
        {filtro_sub.replace("u.cod_subregiao", "u.cod_subregiao")}
        GROUP BY sr.codigo_regiao
    ),
    dados AS (
        SELECT
            sr.codigo_regiao AS cod_regiao,
            ...
            COALESCE(ap.aprendizes, 0) AS aprendizes
        ...
        LEFT JOIN aprendizes_regiao ap
            ON ap.cod_regiao = sr.codigo_regiao
        JOIN programas p ON p.codigo = t.cod_programa

        WHERE UPPER(TRIM(p.nome_programa)) = 'JOVEM APRENDIZ'
        AND mm.ano = $1
        AND mm.mes = ANY($2::int[])
        {filtro_sub}

        GROUP BY sr.codigo_regiao, rg.nome, ap.aprendizes
    )
    SELECT
    ...
    WITH base AS (
        SELECT
            ...
            (
                SELECT COALESCE(SUM(rp.matriculas_real), 0)
                FROM realizado_programas rp
                JOIN ofertas_programas o
                ON o.codigo = rp.cod_oferta
                JOIN programas p
